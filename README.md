...
6. gocryptfs will try unlocking the /encrypted directory and provide unencrypted /usr/src/app/upload (environment variable gocryptfs_decrypt_dir from compose file)
7. loop: decrypt.sh will listen for the password on port 9000 (will continue listening until unlock succeeded)
8. it will try unlocking the encrypted dir
9. send the string "Successfully decrypted" if further connection received  `gocryptfs -allow_other -extpass  "nc" -extpass="-l" -extpass="-p" -extpass "9000" /encrypted $DIR_DECRYPTED`
10. start immich server: `tini -- /usr/src/app/server/bin/start.sh`
...
### build
  ...
  1. get latest Immich Image `docker pull ghcr.io/immich-app/immich-server:release`
...
Docker services are meant to run detached from you rconsole or keyboard/mouse input, therefore we need to pass it to an intermediate service.

`./2_enter-password.sh`

1. This will spin up a sidecar container, you can enter your password there, it will transfer it to the Immich-Container's unlock script for gocryptfs.
2. You can leave this open in case Immich restarts or close it to "forget" the password (safer)

### todo

a more brief explaination shall be added
